...
test.describe('Session timer', () => {
  test('runs from the start of the quiz and is presented calmly', async ({ page }) => {
    const quiz = new QuizPage(page);

    await quiz.goto();
    await expect(quiz.timerLabel()).toBeVisible();
    await expect(quiz.timerLabel()).toHaveText(/^(10:00|9:5\d)$/);
    await expect(page.getByText('Take your time')).toBeVisible();
    // Reassurance, not a deadline: nothing is announced while time remains.
    await expect(page.getByRole('button', { name: 'Add 5 minutes' })).toHaveCount(0);
  });

  test('does not restart when the page is reloaded', async ({ page }) => {
    const quiz = new QuizPage(page);
    const seconds = async (): Promise<number> => {
      const [m, s] = (await quiz.timerLabel().textContent())!.split(':');
      return Number(m) * 60 + Number(s);
    };

    await quiz.goto();
    await quiz.answer('25–34');
    // Let enough time pass that a restart would be unmistakable.
    await page.waitForTimeout(3000);
    const before = await seconds();
    expect(before).toBeLessThan(10 * 60);

    await page.reload();
    await expect(quiz.timerLabel()).toBeVisible();
    // Still counting down from the original start, not reset by the reload.
    expect(await seconds()).toBeLessThanOrEqual(before);
  });

  test('stops once the products reach the cart', async ({ page }) => {
    const quiz = new QuizPage(page);
    ...
    // Returning to the quiz shows the result, with no running timer.
    await page.goto('/quiz');
    await expect(quiz.timerLabel()).toHaveCount(0);
  });
});
